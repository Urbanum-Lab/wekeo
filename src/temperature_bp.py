...
for inpath in dirs:
    ...
    geodetic = xr.open_dataset(filename_or_obj=os.path.join(inpath, "geodetic_tx.nc"), engine='netcdf4')
    ...
    met_tx = xr.open_dataset(filename_or_obj=os.path.join(inpath, "met_tx.nc"), engine='netcdf4')
    temp = met_tx.data_vars["temperature_tx"].to_numpy().flatten()
    # LST_ancillary_ds.nc -> NDVI (empyt :()
    lst = xr.open_dataset(filename_or_obj=os.path.join(inpath, "LST_ancillary_ds.nc"), engine='netcdf4')
